...
public class TooltipsConfig extends Config {
    ...
    public Optional<String> getTooltipString(Item item) {
        String itemRegistryName = Registry.ITEM.getId(item).toString();
        return this.tooltips.stream().filter((entry) -> entry.item.equals(itemRegistryName)).map(TooltipsConfig.TooltipEntry::getValue).findFirst();
    }
    ...
    protected void reset() {
        this.tooltips.clear();
        this.tooltips.add(new TooltipsConfig.TooltipEntry(Registry.ITEM.getId(Items.STONE).toString(), "Stone... It is a block.\nThe most common block.\nOr maybe dirt is...\nI dunno..."));
    }
